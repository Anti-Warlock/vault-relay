...
它可以把当前 Markdown 文章整理成平台草稿。升级 Creator Pro 后，它可以配合浏览器扩展 VaultRelay Companion，把标题、正文和图片填入 X、知乎、微信公众号和小红书的编辑器。

最终发布仍然由你手动点击。VaultRelay 不保存平台账号、密码、Cookie 或验证码。

官网：https://official.vaultrelay.workers.dev/
...
- Creator Pro 可通过 VaultRelay Companion 自动填入平台编辑器。
- Creator Pro 可一键推送到已启用的平台。
...
如果要使用自动填入 X、知乎、公众号或小红书，需要同时安装 VaultRelay Companion，并激活 Creator Pro。
...
3. 选择要推送的平台。
4. 免费版可以复制文本、复制富文本，或导出图片卡片。
5. 激活 Creator Pro 后，可以点击对应平台的推送按钮，或运行 `VaultRelay: 一键 Push 已启用平台`。
6. 浏览器会打开目标平台编辑器，Companion 会自动填入标题、正文和图片。
7. 检查草稿内容。
8. 确认无误后，手动点击平台的发布按钮。
...
https://official.vaultrelay.workers.dev/
